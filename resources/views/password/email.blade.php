      ...
      <div class="bg-white rounded-md w-full max-w-md px-4 sm:px-6 lg:px-8">
        <div class="mt-12">
          ...
          <form method="POST" action="{{ route('password.email') }}" class="mt-4">
            ...
            <input type="email" name="email" placeholder="{{ __('emails.email_placeholder') }}" class="w-full px-6 py-4 mt-3 rounded-md border-gray-300 border-solid border focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 @error('email') is-invalid @enderror" value="{{ old('email') }}" required autocomplete="email" autofocus>
            @error('email')
                            <div class="flex gap-3 items-center mt-2">
                                ...
                                <p class="text-red-500 text-xs italic mt-2">{{__('validation.invalid_email')}}</p>
                            </div>
                        @enderror
